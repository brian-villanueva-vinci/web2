...
if (
  ...
  title?.length === 0 ||
  link?.length === 0 ||
  isNaN(duration) ||  // Utilisation de isNaN pour accepter les nombres entiers
  isNaN(budget)
) {
  // Données invalides, renvoie un statut 400 (Bad Request)
  return res.sendStatus(400);
}
  ...
  if (foundIndex < 0) {
    // Film non trouvé, renvoie un statut 404 (Not Found)
    return res.sendStatus(404);
  }

  const updatedFilm = { ...FILMS[foundIndex], ...req.body };

  // Mise à jour du film dans le tableau FILMS
  FILMS[foundIndex] = updatedFilm;

  // Renvoie le film mis à jour en tant que réponse
